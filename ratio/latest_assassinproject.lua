...
function corsa:is_visible(start, result, part)
	...
	if surge then
		return #workspace.CurrentCamera:GetPartsObscuringTarget({start, result}, {client:get_character(), part, workspace.Instances.KnifeHost, workspace.Instances.Pets}) == 0
	end
end
...
local uiCorner = Instance.new("UICorner",sessionBack2); uiCorner.CornerRadius = UDim.new(0,6)

local win = lib:create_window()

lib.flags["game_id"] = tostring(game.GameId)
...
old_nc = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(self, ...)
    ...
    elseif not checkcaller() and method == "FireServer" and tostring(self) == "KillPlayer" then
        coroutine.wrap(function()
...
local on_render = connection.new(services.RunService.RenderStepped, LPH_JIT_MAX(function()
	...
	if pets then
		local pets = workspace.Instances.Pets:GetChildren()
		for i = 1, #pets do
			local pet = pets[i]
			if pet then
		...
		for i = 1, #pets do
			local pet = pets[i]
			if pet then
				pet.Parent = workspace.Instances.Pets
			end
		end
	end
	local ragdolls = quik.find(lib.flags["removals"]["selected"], "ragdolls")
	if ragdolls then
		local ragdolls = workspace.Instances.Ragdolls:GetChildren()
		for i = 1, #ragdolls do
		...
		for i = 1, #ragdolls do
			local ragdoll = ragdolls[i]
			if ragdoll then
				ragdoll.Parent = workspace.Instances.Ragdolls
			end
		end
	end
			...
			if hrp and hum then
				...
				elseif not visible then
					if lib.flags["offscreen"]["toggle"] then
						local triangle = esp_table.triangle
						local to = esp_table.triangleoutline
						...
						to.PointA = triangle.PointA
						to.PointB = triangle.PointB
						to.PointC = triangle.PointC
						to.Visible = lib.flags["outlines"]["toggle"]
						to.Color = Color3.fromRGB(0,0,0)
...
local on_hit_player = connection.new(corsa.signals.hit_player, function(name)
	...
	if surge then
		...
		if lib.flags["hit_chams"]["toggle"] and character and surge:is_loaded() then
			...
			char_clone.Name = "\255"
			char_clone.Parent = workspace.Instances.KnifeHost
...
getrenv().tick = newcclosure(function(...)
    ...
    if not checkcaller() and s and tostring(s) == "knifeScript" and lib.flags["fastthrow"]["toggle"] then
        if not fake_tick then
            return old() + (34*(lib.flags["fastthrow"]["value"])/100)/100
        else
            fake_tick = false
        end
    end
...
local on_heartbeat = connection.new(services.RunService.Heartbeat, LPH_JIT_MAX(function()
	...
	corsa.info.target = services.Players:FindFirstChild(client.plr.PlayerGui.ScreenGui.UI.Target.PlayerText.Text)
	local character = client:get_character()
	...
	if character and client:is_character_loaded() then
		...
		if fakelag.get_active() and lib.flags["fakelag"]["toggle"] then
			corsa.assets.fake.Parent = workspace.Instances.KnifeHost
			for _, part in pairs(corsa.assets.fake:GetChildren()) do
