...
local menu_general = menu.create_section("rage", "General")
    local menu_triggerbot = menu_general.create_element({name = "Trigger bot", types = {"toggle", "dropdown"}, multi = true, flag = "triggerbot", options = {"Need tool", "Include NPCs", "Ignore forcefield", "Ignore players"}, callback = function(f)
    end})
	local menu_triggerdelay = menu_general.create_element({name = "Delay", types = {"slider"}, flag = "triggerdelay", suffix = "ms", min = 10, max = 250, sdefault = 30, callback = function(f)
    end})

local menu_character = menu.create_section("aa", "Local character")
    local menu_spinbot = menu_character.create_element({name = "Spinbot", types = {"toggle", "slider"}, flag = "spinbot", suffix = "°", min = 1, max = 50, sdefault = 5, callback = function(f)
    end})

local menu_movement = menu.create_section("aa", "Movement")
	local menu_cfspeed = menu_character.create_element({name = "CFrame speed", types = {"toggle", "slider", "keybind"}, kdefault = {["key"] = "G", ["active"] = false, ["method"] = "hold"}, flag = "speed", min = 1, max = 15, sdefault = 1, callback = function(f)
	end})
...
local menu_eesp = menu.create_section("visuals", "Enemy esp")
	...
	local menu_eweapon = menu_eesp.create_element({name = "Weapon", types = {"toggle", "colorpicker"}, flag = "eweapon", cdefault = Color3.fromRGB(255,255,255), callback = function(f)
    end})
    local menu_ehealth = menu_eesp.create_element({name = "Health", types = {"toggle", "colorpicker"}, flag = "ehealth", cdefault = Color3.fromRGB(0,255,0), callback = function(f)
    end})
...
local menu_hud = menu.create_section("visuals", "Game")
	local menu_indicators = menu_hud.create_element({name = "Indicators", types = {"toggle", "dropdown"}, flag = "indicators", options = {"AJ","NC","S"}, multi = true, callback = function(f)
    end})
...
cheat = {trigger_cooldown = false, connections = {},desync = nil, oldcf = CFrame.new(), target = nil}
...
function cheat:add_esp(player)
	LPH_JIT_MAX(function()
		...
		do
			...
			esp.drawings.weapon.Size = 14
			esp.drawings.weapon.Center = true
			esp.drawings.weapon.Outline = true
			esp.drawings.weapon.Font = Drawing.Fonts.Plex
			esp.drawings.weapon.Visible = false
			esp.drawings.weapon.ZIndex = 3
            ...
            esp.drawings.healthoutline = Drawing.new("Line")
			esp.drawings.healthoutline.Color = Color3.fromRGB(0,0,0)
			esp.drawings.healthoutline.Thickness = 3
			esp.drawings.healthoutline.Visible = false
			esp.drawings.healthoutline.ZIndex = 1
			esp.drawings.healthbar = Drawing.new("Line")
			esp.drawings.healthbar.Thickness = 1
			esp.drawings.healthbar.Visible = false
			esp.drawings.healthbar.ZIndex = 2
		end
		...
		esp.connection = cheat:add_connection(services.RunService.RenderStepped, function()
			if player.Parent ~= nil then
				...
				if lib.flags["eesp"]["toggle"] then
					...
					local p_char = workspace:FindFirstChild("PsuedoCharacter"..tostring(player.UserId))
					local gun = nil
					if p_char and p_char:FindFirstChild("GunTool") then
						local tool = p_char.GunTool:FindFirstChild("Gun")
						if tool then
							gun = tool.Value
						end
					end
					...
					local c_team = client.plr.Team
					local team_check = true

					if char and team_check then
						local torso = char:FindFirstChild("HumanoidRootPart")
                        local hum = char:FindFirstChild("Humanoid")
                        if torso and hum and client.loaded() and (torso.Position-client.character().HumanoidRootPart.Position).magnitude < 1500 then
                            ...
                            if visible then
                                ...
                                if lib.flags["echams"]["toggle"] then
                                    ...
                                    esp.highlight.FillColor = lib.flags["echams"].Color
                                    esp.highlight.OutlineColor = lib.flags["eoutline"].Color
                                    esp.highlight.FillTransparency = lib.flags["echams"].Transparency
                                    esp.highlight.OutlineTransparency = lib.flags["eoutline"].Transparency
                                end
                                ...
                                if lib.flags["ebox"]["toggle"] then
                                    ...
                                    esp.drawings.box.Color = lib.flags["ebox"].Color
                                    esp.drawings.box.Transparency =	-lib.flags["ebox"].Transparency+1
                                    esp.drawings.outline.Transparency = -lib.flags["ebox"].Transparency+1
                                ...
                                if lib.flags["ename"]["toggle"] then
                                    ...
                                    esp.drawings.name.Color = lib.flags["ename"].Color
                                    esp.drawings.name.Transparency =  -lib.flags["ename"].Transparency+1
                                    esp.drawings.name.Font = Drawing.Fonts[font]
                                    esp.drawings.name.Size = lib.flags["efont2"]["value"]
                                    esp.drawings.name.Visible = true
                                end

								if lib.flags["eweapon"]["toggle"] then
                                    esp.drawings.weapon.Text = player.Name 
                                    esp.drawings.weapon.Position = Vector2.new(box_size.X / 2 - box_pos.X, box_pos.Y + esp.drawings.name.TextBounds.Y - 1)
                                    esp.drawings.weapon.Color = lib.flags["eweapon"].Color
                                    esp.drawings.weapon.Transparency =  -lib.flags["eweapon"].Transparency+1
                                    esp.drawings.weapon.Font = Drawing.Fonts[font]
                                    esp.drawings.weapon.Size = lib.flags["efont2"]["value"]-1
                                    esp.drawings.weapon.Visible = true
                                end

                                if lib.flags["ehealth"]["toggle"] then
									esp.drawings.healthbar.From = Vector2.new((box_pos.X - 5), box_pos.Y + box_size.Y)
									esp.drawings.healthbar.To = Vector2.new(esp.drawings.healthbar.From.X, esp.drawings.healthbar.From.Y - (char.Humanoid.Health / char.Humanoid.MaxHealth) * box_size.Y)
									esp.drawings.healthbar.Color = lib.flags["ehealth"].Color
									esp.drawings.healthbar.Transparency = -lib.flags["ehealth"].Transparency+1
									esp.drawings.healthbar.Visible = true
					
									esp.drawings.healthoutline.From = Vector2.new(esp.drawings.healthbar.From.X, box_pos.Y + box_size.Y + 1)
									esp.drawings.healthoutline.To = Vector2.new(esp.drawings.healthbar.From.X, (esp.drawings.healthbar.From.Y - 1 * box_size.Y) -1)
                                ...
                                if lib.flags["eoof"]["toggle"] then
									...
									local value = lib.flags["eoof"]["value"]/100
                    
                                    local arrowOrigin = viewport/2 + Vector2.new(cx * big * 75/200, sy * small * 75/200) * lib.flags["eoof2"]["value"]/1000
                                    ...
                                    esp.drawings.triangle.Color = lib.flags["eoof"].Color
                                    esp.drawings.triangle.Transparency = -lib.flags["eoof"].Transparency+1
                                end
                            end
                        end
					else
...
local nc = cheat:create_indicator("NC")
local s = cheat:create_indicator("S")
...
LPH_JIT_MAX(function()
	cheat:add_connection(services.RunService.Heartbeat, function()
		if client.loaded() then
			...
			if lib.flags["speed"]["toggle"] and menu_cfspeed:get_active() then
				client.character().HumanoidRootPart.CFrame = client.character().HumanoidRootPart.CFrame + (client.character().Humanoid.MoveDirection / 50) * lib.flags["speed"]["value"]
			end
			if lib.flags["quickstop"]["toggle"] and client.character().Humanoid.MoveDirection == Vector3.new() then
				client.character().HumanoidRootPart.Velocity = Vector3.new(0,client.character().HumanoidRootPart.Velocity.Y,0)
			end
			...
			nc.Visible = (lib.flags["noclip"]["toggle"] and menu_noclip.get_active() and table.find(lib.flags["indicators"]["option"], "NC"))
			s.Visible = (lib.flags["speed"]["toggle"] and menu_cfspeed.get_active() and table.find(lib.flags["indicators"]["option"], "S"))
	...
	cheat:add_connection(services.RunService.Stepped, function()
		if client.loaded() and (lib.flags["noclip"]["toggle"] and menu_noclip.get_active()) then
