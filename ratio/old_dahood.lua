...
local client = {}
    ...
    client.bodyCache = {}; client.animations = {}; client.target = nil
    ...
    local combat = menu.tab("Combat")
        local legit = combat.subtab("Legit")
            local aim = legit.section("Aim")
                ...
                local prediction = aim.element({name = "Prediction multiplier", type = "slider", min = 90, default = 95, max = 250, flag = "prediction", callback = function()
                end})
                local aimpart = aim.element({name = "Aim at", type = "dropdown", default = false, multi = false, options = {"Head", "HumanoidRootPart"}, flag = "aimpart", callback = function()
                end})
                local silentconditions = aim.element({name = "Don't aim on", type = "dropdown", default = false, multi = true, options = {"Grabbed", "Ragdoll"}, flag = "silentconditions", callback = function()
                end})
    ...
    local visuals = menu.tab("Visuals")
        local esp = visuals.subtab("ESP")
            local enemy = esp.section("Enemy")
                ...
                local enemyespdistance = enemy.element({name = "Max distance", type = "slider", min = 100, default = 250, max = 1000, flag = "espdistance", callback = function()
                end})
        ...
        local ccclient = visuals.subtab("Client")
            ...
            local world = ccclient.section("World")
                ...
                end}); forcefieldbody.addon({name = "Body Colorpicker", remove = false, type = "colorpicker", default = Color3.fromRGB(0,155,175), callback = function(t)
                end}) 
                local forcefieldhat = world.element({name = "Forcefield hats", flag = "forcefieldhat", type = "toggle", callback = function(o)
                end}); forcefieldhat.addon({name = "Hat Colorpicker", remove = false, type = "colorpicker", default = Color3.fromRGB(0,255,175), callback = function(t)
                end}) 
                ...
                local colorshifttop = world.element({name = "Color shift top", flag = "colorshifttop", type = "toggle", callback = function(o)
                end}); colorshifttop.addon({name = "Shift Top Colorpicker", remove = true, type = "colorpicker", default = Color3.fromRGB(0,0,0), callback = function(t)
                end}) 
                local colorshiftbottom = world.element({name = "Color shift bottom", flag = "colorshiftbottom", type = "toggle", callback = function(o)
                end}); colorshiftbottom.addon({name = "Shift Bottom Colorpicker", remove = true, type = "colorpicker", default = Color3.fromRGB(0,0,0), callback = function(t)
                end}) 
    ...
    local playerss = menu.tab("Players")
        local ccccclient = playerss.subtab("Local")
            local character = ccccclient.section("Character")
                ...
                local spinspeed = character.element({name = "Spin speed", type = "slider", min = 1, default = 1, max = 20, flag = "spinspeed", callback = function()
                end})
            local movement = ccccclient.section("Movement")
                ...
                local jumpcooldown = movement.element({name = "No jump cooldown", flag = "jumpcooldown", type = "toggle", callback = function(o)
                end})
...
local function aimChecks(person)
	local bodyeffects = person:FindFirstChild("BodyEffects")
	if bodyeffects then
		local ko = bodyeffects:FindFirstChild("K.O")
		if ko and table.find(lib.flags["silentconditions"], "Ragdoll") and ko.Value == true then
			return false
		end
		local grabbed = bodyeffects:FindFirstChild("Grabbed")
		if grabbed and table.find(lib.flags["silentconditions"], "Grabbed") and grabbed.Value == true then
			return false
		end
	end
	return true
...
for i,v in pairs(services.Players:GetPlayers()) do
    if v ~= client.plr then
        ...
        if workspace.Players:FindFirstChild(v.Name) then
            highlight.Adornee = workspace.Players[v.Name]
        end
		...
		for _, Player in next, services.Players:GetPlayers() do
			if Player ~= client.plr and Player.Character then
				...
				if playerPart and playerHumanoid then
					...
					if onScreen and (client.character().HumanoidRootPart.Position-playerPart.Position).magnitude < 220 then
						...
						local checks = #workspace.CurrentCamera:GetPartsObscuringTarget({client.character().HumanoidRootPart.Position, playerPart.Position}, {workspace.CurrentCamera, client.character(), playerPart.Parent}) == 0
						if hitTargMagnitude < closestTarg and hitTargMagnitude <= threshold and aimChecks(Player.Character) and checks then
...
LPH_JIT_MAX(function()
	...
	old_index = hookmetamethod(game, "__index", function(self, index)
        if not checkcaller() and self == client.mouse and (index == "Hit" or index == "Target") then
			if client.loaded() and lib.flags["silentaim"] and lib.flags["1silentaim"].active and client.target ~= nil and #lib.flags["aimpart"] == 1 and services.Players[client.target].Character:FindFirstChild(lib.flags["aimpart"][1]) then
				local Hit = resolve(services.Players[client.target].Character:FindFirstChild(lib.flags["aimpart"][1]))
				return (index == "Hit" and Hit or services.Players[client.target].Character:FindFirstChild(lib.flags["aimpart"][1]))
			end
        end
...
LPH_JIT_ULTRA(function()
    old = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if not checkcaller() and tostring(self) == "MainEvent" and (args[1] == "TeleportDetect" or args[1] == "CHECKER_1" or args[1] == "CHECKER_2" or args[1] == "OneMoreTime") then
            return
        elseif not checkcaller() and tostring(self) == "Run Service" and method == "IsStudio" then
            return wait(9e9)
        elseif not checkcaller() and method == "IsA" and string.find(self.ClassName, "Body") then
            return wait(9e9)
        end
        return old(self, ...)
    end)
...
LPH_JIT_MAX(function()
    services.RunService.RenderStepped:Connect(function()
        ...
        services.RunService.RenderStepped:Wait()
        if lib.flags["jumpcooldown"] and client.loaded() then
            ...
            if player ~= client.plr then
                if client.esp[player.Name] then
                    ...
                    if lib.flags["enemyesp"] then
                        ...
                        if visible and hrp and hum then
                            ...
                            if lib.flags["enemyesphealth"] then
                                ...
                                playerTable.healthbar.To = Vector2.new(playerTable.healthbar.From.X, playerTable.healthbar.From.Y - (character.Humanoid.Health / character.Humanoid.MaxHealth) * BoxSize.Y)
...
services.Players.PlayerAdded:Connect(function(player)
    ...
    if workspace.Players:FindFirstChild(player.Name) then
        game.CoreGui[player.Name].Adornee = workspace.Players[player.Name]
    end
...
LPH_JIT_MAX(function()
    services.RunService.Heartbeat:Connect(function()
        ...
        game.Lighting.ColorShift_Top = lib.flags["1colorshifttop"].Color
        game.Lighting.ColorShift_Bottom = lib.flags['1colorshiftbottom'].Color
        ...
        if client.loaded() then
            ...
            for i,v in pairs(client.character():GetChildren()) do
                if v:IsA("MeshPart") then
                    ...
                    v.Material = lib.flags["forcefieldbody"] and Enum.Material.ForceField or Enum.Material.SmoothPlastic
                elseif v:IsA("Accessory") then
                    local part = v:FindFirstChildOfClass("Part") or v:FindFirstChildOfClass("MeshPart")
                    if part then
                        part.Color = lib.flags["forcefieldhat"] and lib.flags["1forcefieldhat"].Color or Color3.fromRGB(255,255,255)
                        part.Material = lib.flags["forcefieldhat"] and Enum.Material.ForceField or Enum.Material.SmoothPlastic
                    end
                end
            end
            if lib.flags["silentaim"] and lib.flags["1silentaim"].active then
                ...
                if not services.Players:FindFirstChild(client.target) then
                    client.target = nil
                end
            else
                client.target = nil
